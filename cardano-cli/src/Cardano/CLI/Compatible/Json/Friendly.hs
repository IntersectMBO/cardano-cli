...
import Cardano.Ledger.Dijkstra.TxCert qualified as Ledger
...
renderConwayCertificate cert =
  case cert of
    ...
    c -> error $ "renderComwayCertificate: unknown certificate" <> show c
...
renderDijkstraCertificate cert =
  case cert of
    ...
    L.UnRegDepositTxCert stakeCredential deposit ->
      "Stake address deregistration"
        .= object
          [ "stake credential" .= stakeCredential
          , "deposit" .= deposit
    ...
    c -> error $ "renderDijkstraCertificate: unknown certificate" <> show c
