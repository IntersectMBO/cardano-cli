...
import Cardano.Api hiding (Certificate, mkTxCertificates)
...
runTransactionBuildCmd
  Cmd.TransactionBuildCmdArgs
    ...
    certsAndMaybeScriptWits <-
      sequence
        [ (,mSwit)
            <$> ( fmap (convertToNewCertificate Exp.useEra) $
...
getPoolDeregistrationInfo
  ...
  -> Certificate era
  -> Maybe PoolId
getPoolDeregistrationInfo era (Certificate cert) =
  case obtainCommonConstraints era $ L.getRetirePoolTxCert cert of
    Just (poolId, _e :: EpochNo) -> Just (StakePoolKeyHash poolId)
    Nothing -> Nothing
...
getDRepDeregistrationInfo e (Certificate cert) =
...
getStakeDeregistrationInfo (Certificate cert) =
...
runTxBuildRaw
  ...
  -> [(Certificate era, Exp.AnyWitness (Exp.LedgerEra era))]
      ...
      return $
        shelleyBasedEraConstraints
          ...
          ( defaultTxBodyContent sbe
              ...
              & setTxCertificates (mkTxCertificates certsAndMaybeScriptWits)
...
runTxBuild
  ...
  mCurrentTreasuryValueAndDonation = do
    ...
    shelleyBasedEraConstraints sbe $ do
      ...
      let certsToQuery = map (convertToOldApiCertificate Exp.useEra) $ fst <$> certsAndMaybeScriptWits
...
getAllReferenceInputs
  ...
  readOnlyRefIns = do
    let txinsWitByRefInputs = map getScriptWitnessReferenceInput spendingWitnesses
        ...
        certsWitByRefInputs = map getReferenceInput certScriptWitnesses
        withdrawalsWitByRefInputs = map (getScriptWitnessReferenceInput . wswScriptWitness) withdrawal
...
getReferenceInput :: Exp.AnyWitness era -> Maybe TxIn
getReferenceInput Exp.AnyKeyWitnessPlaceholder = Nothing
getReferenceInput Exp.AnySimpleScriptWitness{} = Nothing
getReferenceInput (Exp.AnyPlutusScriptWitness (Exp.PlutusScriptWitness _ (Exp.PReferenceScript ref) _ _ _)) = Just ref
getReferenceInput (Exp.AnyPlutusScriptWitness (Exp.PlutusScriptWitness _ (Exp.PScript{}) _ _ _)) = Nothing
