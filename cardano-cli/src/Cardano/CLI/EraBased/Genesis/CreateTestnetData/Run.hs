  ...
  Cmd.GenesisCreateTestNetDataCmdArgs
    ...
    shelleyGenesisInit <-
      fromMaybe shelleyGenesisDefaults
        <$> traverse decodeShelleyGenesisFile specShelley
    alonzoGenesis <-
      fromMaybe (alonzoGenesisDefaults era)
        <$> traverse (decodeAlonzoGenesisFile (Just era)) specAlonzo
    conwayGenesis <-
      fromMaybe conwayGenesisDefaults <$> traverse decodeConwayGenesisFile specConway
