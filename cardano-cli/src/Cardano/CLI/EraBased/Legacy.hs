...
parseLegacyCmds envCli =
  Opt.hsubparser $ mconcat
    ...
    , Opt.command "address"
        $ Opt.info (AddressCmds <$> pAddressCmd envCli)
        $ Opt.progDesc "Payment address commands"
    , Opt.command "stake-address"
        $ Opt.info (StakeAddressCmds <$> pStakeAddressCmd envCli)
        $ Opt.progDesc "Stake address commands"
    , Opt.command "key"
        $ Opt.info (KeyCmds <$> pKeyCmd)
    ...
    , Opt.command "node"
        $ Opt.info (NodeCmds <$> pNodeCmd)
        $ Opt.progDesc "Node operation commands"
    , Opt.command "stake-pool"
        $ Opt.info (PoolCmds <$> pPoolCmd envCli)
        $ Opt.progDesc "Stake pool commands"
    , Opt.command "query"
        $ Opt.info (QueryCmds <$> pQueryCmd envCli) . Opt.progDesc
        $ mconcat
    ...
    , Opt.command "genesis"
        $ Opt.info (GenesisCmds <$> pGenesisCmd envCli)
        $ Opt.progDesc "Genesis block commands"
    , Opt.command "governance"
        $ Opt.info (GovernanceCmds <$> pGovernanceCmd envCli)
        $ Opt.progDesc "Governance commands"
    , Opt.command "text-view"
        $ Opt.info (TextViewCmds <$> pTextViewCmd) . Opt.progDesc
        $ mconcat
...
pTextViewCmd :: Parser TextViewCmds
pTextViewCmd =
  asum
...
pAddressCmd :: EnvCli -> Parser AddressCmds
pAddressCmd envCli =
   asum
     [ subParser "key-gen"
...
pStakeAddressCmd :: EnvCli -> Parser StakeAddressCmds
...
pPoolCmd :: EnvCli -> Parser PoolCmds
pPoolCmd  envCli =
  asum
    [ subParser "registration-certificate"
...
pQueryCmd :: EnvCli -> Parser QueryCmds
pQueryCmd envCli =
  asum
...
pGovernanceCmd :: EnvCli -> Parser GovernanceCmds
...
pGenesisCmd :: EnvCli -> Parser GenesisCmds
pGenesisCmd envCli =
  asum
