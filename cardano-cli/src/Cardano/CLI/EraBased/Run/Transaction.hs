...
runTransactionCalculatePlutusScriptCostCmd
  Cmd.TransactionCalculatePlutusScriptCostCmdArgs
    ...
    let Tx txBody _ = tx
    let txBodyContent = getTxBuildBodyContent txBody

    let mScriptWits =
          monoidForEraInEon @AlonzoEraOnwards
            era'
            (\aeo -> collectTxBodyScriptWitnesses (convert aeo) txBodyContent)
...
getTxBuildBodyContent :: TxBody era -> TxBodyContent BuildTx era
getTxBuildBodyContent = undefined -- ToDo: in cardano-api
