...
data TxCmdError
  ...
  | TxCmdEraConsensusModeMismatch
      ...
      !AnyCardanoEra
      -- ^ Era
  | TxCmdBootstrapWitnessError !ShelleyBootstrapWitnessError
...
runTransactionCmd :: TransactionCmd era -> ExceptT TxCmdError IO ()
runTransactionCmd = \case
...
runTxBuildCmd
  ...
  -- Conway related
  votes <- newExceptT $ first TxCmdVoteError
  ...
  case outputOptions of
    OutputScriptCostOnly fp -> do
      ...
      executionUnitPrices <- pure (protocolParamPrices pparams) & onNothing (left TxCmdPParamExecutionUnitsNotAvailable)
      ...
      case consensusMode of
        CardanoMode -> do
          ...
          -- We cannot use the user specified era to construct a query against a node because it may differ
          -- from the node's era and this will result in the 'QueryEraMismatch' failure.
          txEraUtxo <- pure (eraCast cEra nodeEraUTxO) & onLeft (left . TxCmdTxEraCastErr)
  ...
  case (consensusMode, cardanoEraStyle era) of
    (CardanoMode, ShelleyBasedEra _sbe) -> do
      void $ pure (toEraInMode era CardanoMode)
        & onNothing (left (TxCmdEraConsensusModeMismatchTxBalance outputOptions
      ...
      -- We cannot use the user specified era to construct a query against a node because it may differ
      -- from the node's era and this will result in the 'QueryEraMismatch' failure.
      txEraUtxo <- pure (eraCast era nodeEraUTxO) & onLeft (left . TxCmdTxEraCastErr)
