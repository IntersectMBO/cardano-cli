...
runTxBuildCmd
    ...
    conwayVotes newConstitutions outputOptions =
  cardanoEraConstraints era $ do
...
runTxBuildRaw
    era mScriptValidity inputsAndMaybeScriptWits readOnlyRefIns txinsc
    mReturnCollateral mTotCollateral txouts mLowerBound mUpperBound
    mFee valuesWithScriptWits certsAndMaybeSriptWits withdrawals
    reqSigners txAuxScripts txMetadata mpparams mUpdateProp = do
  let allReferenceInputs = getAllReferenceInputs
                              inputsAndMaybeScriptWits
                              (snd valuesWithScriptWits)
...
getAllReferenceInputs txins mintWitnesses certFiles withdrawals readOnlyRefIns = do
  ...
  where
    getReferenceInput :: ScriptWitness witctx era -> Maybe TxIn
...
runTxCalculateMinFeeCmd :: ()
  ...
  -> ExceptT ShelleyTxCmdError IO ()
runTxCalculateMinFeeCmd
    (File txbodyFilePath) nw pParamsFile (TxInCount nInputs) (TxOutCount nOutputs)
    (TxShelleyWitnessCount nShelleyKeyWitnesses) (TxByronWitnessCount nByronKeyWitnesses) = do
  ...
  case unwitnessed of
    UnwitnessedCliFormattedTxBody (InAnyCardanoEra era txbody) -> do
      witnesses <-
        sequence
          [ do
              InAnyCardanoEra era' witness <- firstExceptT ShelleyTxCmdCddlWitnessError . newExceptT
                                                  $ readFileTxKeyWitness file
              case testEquality era era' of
                Nothing   -> left $ ShelleyTxCmdWitnessEraMismatch (AnyCardanoEra era) (AnyCardanoEra era') witnessFile
